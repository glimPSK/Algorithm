/*
Q)  모든 승객 성공적으로 데려다 줄수 있는지? / 최종적으로 남는 연료양 출력
S)  1. 손님 위치 + 목적지 위치
    2. 손님 번호 1~ M 까지 / 벽 1 -> -1로 변경  
    3. 거리가 가까운 사람일 수록 가장 우선적으로 선택
    4. 1번이 여러명일 경우, 행이 가장 작은 사람이 우선적으로 선택
    5. 2번이 여러명일 경우, 열이 가장 작은 사람이 우선적으로 선택 되어야 한다.
